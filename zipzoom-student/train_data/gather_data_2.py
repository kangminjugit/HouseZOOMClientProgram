...
gesture = {
    ...
    6:'X' , 7:'O'
} # 9가지의 제스처, 제스처 데이터는 손가락 관절의 각도와 각각의 라벨을 뜻한다.
    ...
    if result.multi_hand_landmarks is not None:
        for num, hand in enumerate(result.multi_hand_landmarks):    
            ...
            if (len(result.multi_handedness) == 2) and (not np.array_equal(rv3, check)) and (not np.array_equal(lv3, check)) :    
                ...
                data_1 = np.concatenate((l_data,r_data),axis = 0)
...
np.savetxt('HouseZOOMClient/train_data/gesture_data_2.csv', file, delimiter=',')